...
def is_duplicated_number(three_digit):
    ...
    result = True if three_digit[0] == three_digit[1] or three_digit[0] == three_digit[2] or three_digit[1] == three_digit[2] else False
    return result
...
def get_not_duplicated_three_digit_number():
    ...
    while is_duplicated_number(str(number)):
        number = get_random_number()
    return number
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    for i, n in enumerate(user_input_number):
        for j, m in enumerate(random_number):
            if n == m:
                if i == j: strikes += 1
                else: ball += 1
    result = [strikes, ball]
